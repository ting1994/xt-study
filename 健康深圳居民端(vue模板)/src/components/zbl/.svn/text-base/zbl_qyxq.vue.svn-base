<template>
<div class="zbl_qyxq" style="overflow-x:hidden">
    <div class="header">
    ...
    <div class="body">
        <div class="personInfo">
            <div class="left-avatar">
                <img v-if='!avatar' src="../../assets/img/zbl_avatar.png" alt="头像">
                <img v-else :src="imgReqUrl + avatar" alt="头像" style="border-radius:50%;">
            </div>
        ...
        <div class="container addressPhone cbafter">
            ...
            <mt-cell title="居住地址" is-link @click.native='clkHomeTown'>
                <span v-if='zbl_address.decText' class="hideMore">{{zbl_address.decText}}</span>
...
export default {
    data: function() {
        return {
            ...
            sex: '',
            sexImg: [
              require('../../assets/img/zbl_male.png'), require('../../assets/img/zbl_female.png')
            ],
            idType: '',
    ...
    methods: {
        ...
        mapAddress() {
            ...
            provinceCityArea.forEach(item => { //匹配省
                if (item.parent === this.zbl_personInfo.province) {
                    province = item.province
                }
            })
            provinceCityArea.forEach(item => { //匹配市
                if (item.regionCode === this.zbl_personInfo.city) {
                    city = item.city
                }
            })
            provinceCityArea.forEach(item => { //匹配区
                if (item.regionCode === this.zbl_personInfo.district) {
                    district = item.district
                }
            })
            let params = `parentKey=${this.zbl_personInfo.district}&sliceType=4`
            areaAjax(params).then(res => {
                this.street = res.items.filter(item => { //匹配街道
                    return item.key === this.zbl_personInfo.street
                })[0].text
                result = province + city + district + this.street + this.zbl_personInfo.address
                // this.hadAddress = result
                let obj = {}
                obj.parentCode = this.zbl_personInfo.province //省编码
                obj.cityCode = this.zbl_personInfo.city //市编码
                obj.regionCode = this.zbl_personInfo.district //区编码
                obj.addressDetail = this.zbl_personInfo.address //用户输入的具体地址
                obj.decText = result //用户输入的完整地址
                obj.streetText = this.street //用户选中的街道
                obj.streetKeyCode = this.zbl_personInfo.street //用户选中的街道编码
                // console.log(obj)
                this.zbl_selectAddress(obj)
            })
        },
        ...mapActions(['zbl_editPhoneNo', 'xieyiCont', 'zbl_getApplyInfo','zbl_selectAddress'])
    },
    created() {},
    mounted() {
        // console.log(this.zbl_personInfo)
        this.personName = this.zbl_personInfo.personName
        this.avatar = this.zbl_personInfo.avatar
