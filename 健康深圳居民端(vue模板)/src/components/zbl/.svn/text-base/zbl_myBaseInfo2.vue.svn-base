<template>
<div class="zbl_mybaseinfo" style="overflow-x:hidden;height:100vh;">
	<div class="header">
	...
	<div class="body">
		...
		<mt-field label="*姓名" v-model='zbl_personName' placeholder="请输入姓名"></mt-field>

		<mt-cell title="*国籍">
			<span style='margin-right:25px;color:grey'>中国</span>
		</mt-cell>

		<mt-cell title="*证件类型">
			<span style='margin-right:25px;color:grey'>居民身份证</span>
		</mt-cell>

		<mt-field label="*证件号码" v-model='zbl_certificateNo' placeholder="请输入证件号码" @change='change'></mt-field>
		...
		<mt-datetime-picker type="date" ref="picker" :startDate="new Date(40,0,1)" :endDate='new Date()' v-model='pickerValue' @confirm="handleConfirm"></mt-datetime-picker>

		<mt-cell title="*手机号码">
			<span style='margin-right:25px;color:grey'>{{zbl_phoneNo}}</span>
		</mt-cell>
	...
	methods: {
		change(value) {
			...
			if (/^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$|^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X|x)$/.test(value)) {
				if(value.length == 15) {
					document.querySelector('.birthDay').innerText = '19' + value.slice(6,8) + '-' + value.slice(8,10) + '-' + value.slice(10,12)
					parseInt(value[value.length - 1]) % 2 == 0 ? document.querySelector('.sexType').innerText = '女' : document.querySelector('.sexType').innerText = '男'
				} else {
					document.querySelector('.birthDay').innerText = value.slice(6,10) + '-' + value.slice(10,12) + '-' + value.slice(12,14)
					parseInt(value[value.length - 2]) % 2 == 0 ? document.querySelector('.sexType').innerText = '女' : document.querySelector('.sexType').innerText = '男'
				}
			} else {
				MessageBox('提示','证件号码输入有误')
			}
		},
		//保存提交
		updateInfo2() {
		...
		openPicker() {
			let date = document.querySelector('.birthDay').innerText
			let year = parseInt(date.substring(0,4))
			let month = parseInt(date.substring(5,7)) - 1
			let day = parseInt(date.substring(8,10))
			if(year < 2000) {
				year = parseInt(date.substring(2,4))
			}
			if(month < 10) {
				month = parseInt(date.substring(6,7)) - 1
			}
